...
if COMPUTE_NUM_TABLE:
    dataset = pickle.load(
        open("Datasets/matlab_direct_expanded_data_channel_comb_to10.pkl", "rb"))
    val_range = pickle.load(open("source/val_range.pkl", "rb"))
    NUM_TABLE = np.zeros(
        [len(set([data[i] for data in dataset])) for i in range(4)])
        ...
        for j_idx, j in enumerate(sorted(set([data[1] for data in dataset]))):
            for k_idx, k in enumerate(sorted(set([data[2] for data in dataset]))):
                for l_idx, l in enumerate(sorted(set([data[3] for data in dataset]))):
                    rows = [row for row in dataset if row[0] ==
                            i and row[1] == j and row[2] == k and row[3] == l]
                ...
                if min_val != 0:
                    ...
                    for val in range(ceil):
                        if abs(val-min_val) >= 10:
                            ...
                            state = (i, j, np.around(
                                DR_TATBLE[k], 2), LEN_TABLE[l], val)
                            action = ACT_TABLE[int(sign)+1]
                            print(action, 1, state)
                            next_state = (i, j, np.around(
                                DR_TATBLE[k], 2), LEN_TABLE[l], int(val-sign))
