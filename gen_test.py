...
class SParaData(Dataset):
    ...
    def __init__(self, opt, choice):
        ...
        for i in range(2):
            for j in range(2):
                for k in np.arange(0.5,10.05,0.097):
                    for l in np.arange(1.5,4.05,0.057):
                        for p in np.arange(1,101,4):
...
pickle.dump(gened_dataset, open("./source/generated_test_dataset.pkl","wb"))